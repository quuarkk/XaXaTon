...
| `text/` | .txt | Чтение с автоопределением кодировки (UTF-8, CP1251, Latin-1) |
| `images/` | .tif, .tiff | OCR (Tesseract + предобработка OpenCV) |
| Любая | .mp4 | Автоматически добавляются в результат без анализа |
...
**Важно:** Скрипт сканирует **только** папки `data/`, `text/`, `images/`. MP4-файлы, найденные в любом месте, добавляются в результат без анализа.
...
1. Извлечение текста из PDF/DOC/HTML требует дополнительных библиотек
2. Основной фокус решения — структурированные данные (CSV/Parquet) и сканы документов (TIF/TIFF)

### Что насчёт MP4-файлов?

MP4-файлы **автоматически добавляются** в `result.csv` без анализа содержимого. Это позволяет:
- Не терять файлы, которые могут содержать ПДн в видеоформате
- При необходимости доработать скрипт для извлечения аудиодорожки и распознавания речи
...
2. **Добавить обработчики** для новых форматов:
```python
...
| `scan.py` | Ищет ПДн в структурированном датасете | `data/`, `text/`, `images/` | `result.csv` + Markdown-отчёт + отладка |
...
4. Предоставить руководству `pdn_report_*.md` для принятия мер по 152-ФЗ
